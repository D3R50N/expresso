...
class MailService {
    ...
    static async sendTemplateMail({ subject, template_path = 'index', template_var = {}, to = [] }) {
        if (!template_path.endsWith(".html")) template_path += ".html";

        var content = fs.readFileSync(path.join(__dirname,"template", template_path), { encoding: "utf-8" });
